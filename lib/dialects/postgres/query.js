  ...
  var onSuccess = function(rows, sql, result) {
    ...
    if (this.send('isSelectQuery')) {
      ...
      if(this.callee !== null) { // may happen for bulk inserts
        var attrsMap = Utils._.reduce(this.callee.attributes, function(m, v, k) { m[v.toLowerCase()] = v; return m}, {})
        for (var key in rows[0]) {
          if (rows[0].hasOwnProperty(key)) {
            var record = rows[0][key]
            var targetAttr = attrsMap[key] || key
            if (!!this.callee.daoFactory && !!this.callee.daoFactory.rawAttributes && !!this.callee.daoFactory.rawAttributes[targetAttr] && !!this.callee.daoFactory.rawAttributes[targetAttr].type && !!this.callee.daoFactory.rawAttributes[targetAttr].type.type && this.callee.daoFactory.rawAttributes[targetAttr].type.type === DataTypes.HSTORE.type) {
              record = hstore.parse(record)
            }
            this.callee.dataValues[targetAttr] = record
          }
        }
      }  
